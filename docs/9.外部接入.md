...
阿里云的API网关: https://help.aliyun.com/product/29462.html

需求是：
现在我们的同步节点，已经有现成的API（http）服务，但是，不能直接对外，需要加一层网关。
这个网关最好不用再开发，用现成的、成熟的、可运维的的方案。
这个网关只要能够实现API鉴权、限流即可。
...
问题：
- 是否保存用户的NFT藏品记录？
  - 只保存用户在BaaS的藏品记录，其他的方式的不记录，可能会导致余额与链上不同
  - 如果保存链上全量NFT数据，数据库会很大
  - 使用区块浏览器的数据库？
- NFT转移的目的地址，一定要是BaaS内的地址吗？ 如果不是，对其他业务是否有影响？比如，DID

- 目前DID的藏品记录是存在数据库中的，并没有解析链上合约中的状态，这样，如果某个DID中的账户接收了一个来自别的地方的藏品，那么在DID也是解析不到的。
  - 如果对外开放奇异链的API，每个B端都按照中心化的方式记录NFT的余额，那么，奇异链上的每个B端仍然是“各自为战”。
    - 建立一个中心化的数据库，存取链上所有NFT的记录？

方案1——纯BaaS服务，不考虑藏品同步到稀物进行二手交易：
...
>注意：需要推送给DID，以便支持在稀物中进行二手交易, 通过此API服务发行的藏品可以在稀中进行二手交易
...
graph TD;
    ...
    subgraph 奇异链BaaS
        DB[(DB)]
        SERVICE(BaaS API服务)
        BaaSAdmin("BaaS管理台(B端)")
    end
    EXT--"API请求(https)"-->SERVICE
    SERVICE-->DID
    SERVICE---DB
    EXT--界面操作-->BaaSAdmin
    BaaSAdmin---DB
...
方案3——考虑藏品和同步到稀物进行二手交易，同时对外封装了链服务
...
这种方案，外部项目相当于SaaS中的一个“租户”

- 外部项目方的C端用户的实名认证，必须经过我们的API进行。


---

方案4——考虑藏品和同步到稀物进行二手交易，同时对外封装了链服务
...
类似[Infura](infura.io) 和 [Alchemy](www.alchemy.com)

Infura和Alchemy都有免费版和付费版，个人开发者和中小项目团队使用他们的免费版API就可以完成开发需求。当然Infura和Alchemy只是做数据节点服务的，虽然用户可以免费使用API，但是链上的Gas消耗需要用户自己承担。

我们的对外免费也是相对的免费，有限度的免费（例如可以采取：免费版限制API调用频率、每日免费Gas额度、每月免费Gas额度等等措施），如果免费版可以为奇异链带来用户量、为稀台SaaS或稀物带来用户增长，这对于奇异链的生态是非常有利的。
